...
async def invoke_hacker_hot(gravity, data_rows):
    # Actually invoke the function
    response = supabase.functions.invoke("hackerhot-many", invoke_options={"body": {
        'gravity': gravity,
        'data_rows': data_rows
    }})
...
def update_scores(gravity):
    ...
    resp_bytes = loop.run_until_complete(invoke_hacker_hot(gravity, query.data))

    # convert bytes data into regular json dictionary
    resp_json = json.loads(resp_bytes.decode('utf-8'))
    scoresDict = resp_json['scores']

    # Iterate through all keys in dictionary
    for key in scoresDict:
        # UPDATE services SET iscore = scoresDict[key] WHERE id = key
        data = supabase.table("services").update({"iscore": scoresDict[key]}).eq("id", key).execute()
